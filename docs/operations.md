...
45a. Run `POST /report-definitions/schedule-runs` with `mode=execute`, `execute_step=deliver`, missing `confirm_delivery=CREATE_DELIVERY_RECORDS`; confirm it is rejected with `400`.
45b. Run `POST /report-definitions/schedule-runs` with `mode=execute`, `execute_step=deliver`, all three confirmation strings, a valid idempotency key, and explicit `allowed_domains` or `allowed_emails` against an eligible test definition; confirm it generates the report and creates one active delivery record without sending email.
45c. Confirm scheduled delivery response, run record, and logs do not include secret, PIN, raw email, token fragments, download URL, Signed URL, IP, user agent, Admin key fingerprint, SQL text, template mapping details, template GCS URI, Excel cell values, provider event JSON, raw idempotency keys, local file paths, or GCS URI.
45d. Confirm scheduled delivery does not create Cloud Scheduler jobs, send email, post download URLs to Slack, or change SQL/template mapping/storage destinations.
45e. For Cloud Scheduler attachment readiness, call `POST /admin/report-definitions/schedule-runs` with an allowed OIDC service account and confirm unauthorized requests fail closed before any report generation or delivery creation.
45f. Confirm the Cloud Scheduler endpoint ignores request-time `allowed_domains` / `allowed_emails` and uses only the persisted report definition domain allowlist for recurring delivery.
