...
定期実行は週1回を目安にします。GitHub Actions の Cloud Run deploy workflow
では、deploy後にこの wrapper を自動実行し、artifactを保存します。
...
単位で確認します。対象 backup が管理フォルダ配下で確認できない、または
...
- monthly folder: `ICE_20230825_report / 月次`
...
上記フォルダURL、operational owner、primary operator を記録します。対象
backup がこの管理フォルダ配下にない場合は例外扱いとし、個別 backup URL、
...
- folder URL、operational owner、primary operator を削除承認記録へ転記する
- 管理フォルダ配下ではない backup は、個別backup URLを必須項目として扱う

Drive検索で対象 backup を管理フォルダ配下に確認できない場合:

1. GCS削除は実施しない
2. Notion削除承認に「Drive backup 管理フォルダ配下未確認」と記録する
3. 個別 backup URL、file name、閲覧確認者、確認日時を記録する
4. 管理フォルダを変更する場合は、別タスクで folder URL、operational owner、
   共有範囲、保持方針を更新する
...
保持期限を満たしている根拠:
Drive backup URL:
Drive backup folder URL / owner: https://drive.google.com/drive/folders/1f4vMoGZnteUlRuejnywZuR0pDtIb6D36 / システム管理室
