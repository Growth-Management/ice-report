...
2026-06-22 時点では、この削除承認テンプレートを専用 Notion DB へ
移行しません。削除承認の実運用件数が少ない段階では、DB schema の維持と
入力項目管理の負荷が先に増えるため、当面は本テンプレートを手動記録へ
貼り付けて使います。
...
- 削除承認が継続的に発生する
- 複数の削除承認を横断検索する必要がある
- 監査上、承認者、実施者、対象、結果を構造化して集計する必要がある
...
- 対象レポートのDrive保存先配下に確認できない場合は、個別 backup URL、
