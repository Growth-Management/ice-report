...
4. read-only wrapper の週次実行結果を3回分確認し、timeout / 権限 / threshold の変更要否を判断: 初回手動runでGitHub Actions権限不足を確認、権限追加後に再実行
...
- `Operations Read-Only Check` workflow の成功run 3回分
  - 2026-06-29 手動run `28352409745` は success。1/3として記録済み
  - 2026-06-30 手動run `28408454687` は success。2/3として記録済み
...
- public service `/admin` 閉鎖またはAdmin専用service限定の再判断は、
  IAP経路での人間操作実績とscript/API移行影響の確認後に実施
- 複数管理者の実追加smokeは、追加対象userが決まった時点で実施
