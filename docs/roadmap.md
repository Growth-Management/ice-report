...
- レポート生成時に published template を利用する切替、SQL編集、template mapping編集、schedule自動化、保存先変更は未実装

Published template runtime switch design:

- Runtime design is tracked in `docs/published-template-runtime-switch.md`.
- The first implementation should only use a published template when an admin explicitly passes `report_id` to `/generate` or `/deliveries` generation.
